...
int main(int argc, char * argv[]) {
  ...
  int num_events_per_bin = 50000; // Number of events to simulate

  // Create a rootfile to store the histograms
  char fname[40];
  sprintf(fname,"er_band_topmid_5-21-2017_0_10.root");
...
//    w.SetTb(0.236);
  ...
  for(int bin=1; bin<41; bin++) {
    ...
    g_lower->SetPoint(bin,(double) n_ph, psd_params[low_ind]);  
    g_median->SetPoint(bin,(double) n_ph, psd_params[med_ind]);  
    g_upper->SetPoint(bin,(double) n_ph, psd_params[hi_ind]);
