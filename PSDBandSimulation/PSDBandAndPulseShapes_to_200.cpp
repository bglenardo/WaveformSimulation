...
int main(int argc, char * argv[]) {
  ...
  int num_events_per_bin = 3000; // Number of events to simulate

  // Create a rootfile to store the histograms
  char fname[40];
  sprintf(fname,"er_band_pulse_shapes_bot_4-13-2017.root");
  TFile * file = new TFile( fname, "RECREATE");
  TGraph waveform;

  TH1F * h_PSD_40 = new TH1F("h_PSD_40","h_PSD_40",100,0.,1.);
  TH1F * h_pulseShape_40 = new TH1F("h_pulseShape_40","h_pulseShape_40",200,-20.,100.);
  TH1F * h_PSD_100 = new TH1F("h_PSD_100","h_PSD_100",100,0.,1.);
  TH1F * h_pulseShape_100 = new TH1F("h_pulseShape_100","h_pulseShape_100",200,-20.,100.);
  ...
  for(int bin=1; bin<69; bin++) {
    ...
    for(int i=0; i<num_events_per_bin; i++) {
      ...
      for( int j=0; j<n_ph; j++){
        ...
        if( n_ph > 100. && n_ph < 110. )
          h_pulseShape_100->Fill( t_ph );
      }
      psd_params.push_back( prompt_sum/total_sum );
      if( n_ph > 40. && n_ph < 50. )
        h_PSD_40->Fill( prompt_sum/total_sum );
      if( n_ph > 100. && n_ph < 110. )
        h_PSD_100->Fill( prompt_sum/total_sum );
